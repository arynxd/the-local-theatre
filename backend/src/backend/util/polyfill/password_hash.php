...
// Credit: https://github.com/ircmaxell/password_compat
/**
...
namespace {
	...
	if (!function_exists('password_hash')) {
		...
		function password_hash($password, $algo, array $options = []) {
			...
			if (isset($options['salt'])) {
				...
				if (
					...
					$strong = false;
					$buffer = openssl_random_pseudo_bytes(
						$raw_salt_len,
						$strong
					);
					if ($buffer && $strong) {
		...
		function password_get_info($hash) {
			...
			if (
				PasswordCompat\binary\_substr($hash, 0, 4) == '$2y$' &&
