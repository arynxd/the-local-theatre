...
export class BackendAction<T> extends Promise<T> {
     ...
     */
	public static new(route: CompiledRoute): BackendAction<Response> {
	 ...
	 */
	public static usingCache<T>(
		cacheSelector: () => T | undefined,
		onMiss: () => BackendAction<T>
	): BackendAction<T> {
		return new BackendAction(async (res, rej) => {
			...
			if (cacheHit) {
				...
				onMiss().then(res).catch(rej)
			}
		})
	}

	public flatMap<U>(
		mapper: (action: T) => BackendActionLike<U>
	): BackendAction<U> {
	...
	public assertTypeOf<U extends keyof ValidTypeOf>(
		type: U
	): BackendAction<ValidTypeOf[U]> {
	...
	private throwIfTypeOfInvalid<U extends keyof ValidTypeOf>(
		value: unknown,
		type: U
	): asserts value is ValidTypeOf[U] {
		if (typeof value !== type) {
			throw new TypeError(
				'Assertion failed, value was not of type ' +
					type +
					' \n Got ' +
...
function newBackendAction(route: CompiledRoute): BackendAction<Response> {
	...
	return new BackendAction<Response>(async (resolve, reject) => {
		...
		if (route.routeData.requiresAuth) {
			if (!auth.token) {
				let msg =
					'Route ' +
					route +
					' requires auth but the AuthManager has no token set.'
		...
		if (result.ok) {
			...
			const ex = new BackendError(
				'JSON response was malformed. Expected object, got ' +
					JSON.stringify(json)
			)
